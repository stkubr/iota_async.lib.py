...
from iota_async import AdapterSpec, Address, BundleHash, ProposedTransaction, Tag, \
    TransactionHash, TransactionTrytes, TryteString, TrytesCompatible
from iota_async.adapter import BaseAdapter, resolve_adapter
from iota_async.commands import BaseCommand, CustomCommand, core, \
    discover_commands, extended
from iota_async.commands.extended.helpers import Helpers
...
class StrictIota(with_metaclass(ApiMeta)):
    ...
    """
    commands = discover_commands('iota_async.commands.core')
    ...
    async def create_command(self, command):
        ...
        :param command:
            The name of the command to create.
        """
        return await CustomCommand(self.adapter, command)
    ...
    async def get_balances(self, addresses, threshold=100):
    ...
    async def get_transactions_to_approve(self, depth):
        # type: (int) -> dict
        """
        ...
        """
        return await core.GetTransactionsToApproveCommand(self.adapter)(depth=depth)
...
class Iota(StrictIota):
    ...
    """
    commands = discover_commands('iota_async.commands.extended')
    ...
    async def broadcast_and_store(self, trytes):
        ...
        """
        return await extended.BroadcastAndStoreCommand(self.adapter)(trytes=trytes)
    ...
    async def get_bundles(self, transaction):
        ...
        :raise:
          - :py:class:`iota_async.adapter.BadApiResponse` if any of the
        ...
        """
        return await extended.GetBundlesCommand(self.adapter)(transaction=transaction)
    ...
    async def get_inputs(
        ...
        :return:
            ...
            .. code-block:: python
                ...
                response = iota_async.get_inputs(...)

                input0 = response['inputs'][0] # type: Address
                input0.balance # 42

        :raise:
            - :py:class:`iota_async.adapter.BadApiResponse` if ``threshold``
