...
from iota_async import Hash, TRITS_PER_TRYTE, TryteString, TrytesCompatible
from iota_async.crypto import FRAGMENT_LENGTH, HASH_LENGTH
from iota_async.crypto.kerl import Kerl
from iota_async.crypto.types import PrivateKey, Seed
from iota_async.exceptions import with_context
from iota_async.trits import add_trits, trits_from_int
...
def validate_signature_fragments(
    ...
    :param fragments:
      ...
      :py:class:`iota_async.transaction.Fragment` instances).

    :param hash_:
      Hash used to generate the signature fragments (usually a
      :py:class:`iota_async.transaction.BundleHash` instance).

    :param public_key:
      The public key value used to verify the signature digest (usually a
      :py:class:`iota_async.types.Address` instance).
